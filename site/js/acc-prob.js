  ...
  function method(meth) {
    ...
    A.methodNote(meth, {
      ...
      rules: [
        ...
        'For an ensemble the distribution is read over every hour of the day still to come, with the hourly errors taken as perfectly correlated. The day’s extreme is above a strike exactly when one of those hours is, so the chance it pays is read from the hour that comes closest, using that hour’s mean and its members’ spread. The day’s extreme is the more extreme of that and what has already been observed. Any strike the observations have not cleared pays only if the hours left reach it, and a contract pays when the unrounded extreme reaches half a degree past the strike, which is how settlement rounds. Nothing is fitted and no bias is removed.',
        'Two caveats belong with the ensembles. A model publishes a spread for each hour, not for the extreme of several hours, and taking the hourly errors as perfectly correlated is the assumption this page makes in place of the members it does not publish; it widens the distribution slightly. And the level bias in each ensemble’s centre, which the mean error in the scorecard shows, passes straight into its probability.',
