  ...
  function method(meth) {
    ...
    A.methodNote(meth, {
      ...
      rules: [
        ...
        'For an ensemble the distribution is a normal curve on the ensemble’s own forecast of the day’s extreme and its own spread at the hour that extreme falls on, and a contract pays when the unrounded extreme reaches half a degree past the strike, which is how settlement rounds. Nothing is fitted and no bias is removed. A strike the observations have already cleared is paid in full for an ensemble, as every value on the page is held at the running observed extreme.',
        'Two caveats belong with the ensembles. A model publishes a spread for each hour, not for the day’s extreme, so reading the peak hour’s spread as the extreme’s is an approximation this page makes. And that spread does not narrow once the hour of the day’s high or low has passed, so late in the day an ensemble can still give a real chance to a strike the day will not reach.',
