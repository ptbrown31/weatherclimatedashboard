/* Figure 1, the lead curve.
   ...
   target day, the ForecastEx prediction market against the alternative forecast systems, on a matched sample
   ...
   is read the way the day is lived. The ForecastEx prediction market is drawn through the bin
   centers with its bootstrap band because a ladder is quoted every ten
   minutes and its error moves smoothly within an hour. A forecast is drawn as a
   step, one flat tread per hourly bin, because an alternative forecast system's value changes only
   when a cycle lands and is otherwise the same record forward filled, so a
   line joining bin centers would draw motion the forecast never had.
   ...
   an alternative forecast system's last live update for the day, so each alternative forecast system's line ends where its
...
window.WXAccLead = (() => {
  ...
  // short zone name for an alternative forecast systemtip line, the full IANA id on the title
  ...
  function render() {
    ...
    // ---- any faint own-span series first, so the main lines sit on top
    const extraLabels = [];
    extras.forEach(id => {
      const o = own[id];
    ...
    // ---- the ForecastEx prediction market band, then the alternative forecast systems, then the ForecastEx prediction market line on top
    ...
    // the ForecastEx prediction market named on the figure, under its own left end where the alternative forecast systems are not
    if (S.FX) {
    ...
    if (ofVaries) {
      ...
      svg.appendChild(txt('live record', { x: g.L - 8, y: s2 + 18.5, 'text-anchor': 'end', class: 'ax', 'font-size': 9 }));
    } else {
    ...
    hs.forEach((hh, i) => {
      ...
      // the beats cell darkens with the share of alternative forecast systems the ForecastEx prediction market came in under
  ...
  /* Which days this view drew on. A sample starts where every
     member has a value, so the line also says how far the ForecastEx prediction market's own
