...
window.WXAccLead = (() => {
  ...
  const FRAMES = [
    { key: 'target', label: 'Own target', title: 'Each system scored against the observation it is built to predict: the METAR settle for the ForecastEx prediction market and for every alternative that issues values at fixed hours, and the National Weather Service climate report for the four that issue a daytime high and overnight low of their own, that forecast and the GFS, NAM and National Blend station guidance, on the days the report puts the extreme inside their window' },
  ...
  function methodNote(container) {
    ...
    A.methodNote(container, {
      ...
      body: [
        ...
        'Four systems are the exception, the National Weather Service forecast and the station guidance from the GFS, the NAM and the National Blend of Models. Each issues a daytime high for 7 AM to 7 PM and an overnight low for 7 PM to 8 AM local standard time rather than a value at every hour, so in the default own-target frame each is scored against the Service’s climate report on the city-days whose report puts the extreme inside that window, for a low between midnight and 8 AM, the part of the window the report’s day covers. On the other city-days those forecasts were not predicting the day’s extreme and are not scored. Buckley Field has no climate report of its own, only Denver’s, so they are not scored there in this frame. The METAR settle frame scores every system against the settle, those four included, and the climate-report frame scores every alternative forecast system against the report on every date.',
