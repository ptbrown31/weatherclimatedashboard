...
REGION_SIZE = (760, 543)
# The images are fetched at twice the size they are drawn at. The service
# renders the same cartography either way, so this is straight supersampling:
# on any screen with more than one device pixel per CSS pixel the map stops
# looking soft, which on a page of hand-drawn vector charts is the one element
# that read as low quality. Logical dimensions below are what the overlay's
# coordinate transform uses and they do not change.
SCALE = 2
...
def fetch_image(lat: float, lon: float, timeout: int = 60,
    ...
    q = {"bbox": bbox(lat, lon, half_w, half_h), "bboxSR": "4326", "imageSR": "3857",
         "size": f"{size[0] * SCALE},{size[1] * SCALE}", "dpi": str(96 * SCALE),
...
def locator_pass(cfg: dict, store: Storage, fetch: Optional[Callable] = None) -> int:
    ...
    for c in roster:
        ...
        # a station whose picture predates a resolution change is refetched
        if have and have.get("scale") != SCALE:
            have = None
            entry = None
        if not (have and store.get(KEY.format(sid=sid))):
            ...
            entry = {"w": SIZE[0], "h": SIZE[1], "halfWKm": HALF_W_KM, "halfHKm": HALF_H_KM,
                     "lat": c["lat"], "lon": c["lon"], "bytes": len(png), "scale": SCALE,
                     "written": _iso(now)}
            written += 1
        else:
            skipped += 1
        ...
        if not (entry.get("region") and entry["region"].get("scale") == SCALE
                and store.get(REGION_KEY.format(sid=sid))):
            try:
                ...
                entry["region"] = {"w": REGION_SIZE[0], "h": REGION_SIZE[1],
                                   ...
                                   "bytes": len(rpng), "scale": SCALE, "written": _iso(now)}
            except Exception as e:  # noqa: BLE001
